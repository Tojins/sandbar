...
// empty issue set. `runPreflight` probes the prerequisites exactly once and
// hands them to `gatherState`, so one run never observes two answers.
//
...
// tracker-backed lookup in this module assumes. `runPreflight` establishes
// them once, ahead of the tracker, and hands them down.
export type TrackerPrerequisites = {
  readonly hasGh: boolean;
  readonly ghAuthOk: boolean;
};

// What `gatherState` reports when the tracker cannot be asked: nothing. The
// prerequisite failure is then the refusal the operator reads, rather than an
// unanswered query dressed up as a branch classification.
const NO_BRANCH_CLASSIFICATION: BranchClassification = {
  unmerged: [],
  discarded: [],
  resumable: [],
  parked: [],
...
export async function gatherState(
  ...
  // Every lookup behind this is a tracker query that propagates a failure
  // (#99), so it is only asked once the prerequisites hold; a missing or
  // unauthenticated `gh` is reported by its own tailored refusal instead.
  const { unmerged, discarded, resumable, parked } = hasGh && ghAuthOk
    ? await classifyTrackerBranches(cfg, repoDir, knownChunkMemberIssues)
    : NO_BRANCH_CLASSIFICATION;
...
// The tracker-backed half of `gatherState`: the branch classification whose
// every input is a `gh` query. Split out so the caller can gate it on the
// prerequisites as one expression, and so the state it produces is built
// once, immutably, rather than accumulated.
async function classifyTrackerBranches(
  cfg: PreflightConfig,
  repoDir: string,
  knownChunkMemberIssues: ReadonlySet<number> | undefined,
  ...
  // asked about state — the rest are already resumable or chunk members.
  ...
  return classifySandbarBranches({
    branches,
    upstreamTracks,
    openReadyIssues,
    chunkMemberIssues,
...
// query for the same reason as above. A `for-each-ref` over the cache that
// fails propagates (#99): "no chunk members" is an answer git never gave, and
// both readers of this set — the delete pass and `gatherState` — would act on
// it.
