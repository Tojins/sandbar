...
// parseCodexJsonLine: its agent-message dialect supplies all of a codex run's
// speech, so a parser change and a CLI change belong to the same driver
// release. Its `turn.completed` usage branch is deliberately softer: a drift
// there loses a measurement but cannot change a verdict or spend a round (#85).
