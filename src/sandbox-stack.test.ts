...
// which containers it contains and where the agent finds a sibling's log. The
// second drives `startSandboxStack`
...
describe("sandboxContainers", () => {
  ...
  // declares nothing gets nothing — no sibling, no log mount, no prompt slot.
  // Asserted on a REALISTIC stack rather than an empty one, because the failure
  // would be a filter that let everything through.
...
describe("startSandboxStack (#44 D3)", () => {
  ...
  it("reports every declared sibling, with its log path", async () => {
    const r = recorder();
