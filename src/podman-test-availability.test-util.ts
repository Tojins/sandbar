...
// measured difference, and the measurement retired its claim.
//
...
export type PodmanTestInputs = {
  ...
  // one cannot stand in for it. Two files ask for it and they do not ask for
  // quite the same reason, which is why the field is not named after either.
  ...
  // socket. Both want the same answer — do not run here — and neither wants a
  // failure.
  //
  // What the flag is NOT for: a file nobody has got round to measuring through
  // a remote client. agent-sandbox-podman.test.ts asked for it on exactly that
  // basis from #48 to #52, and the measurement then said the assertions carry
  // — so the flag had been holding a whole file out of the gate on an
  // assumption. Ask for it because a remote client demonstrably does something
  // else, and say which thing in the file's header.
