// Config-declared image builds and the one image property the gate stack cannot
// discover at run time without breaking.
//
...
//
// EVERY build entry point RECORDS what it did (#82). Both entry points here —
// `ensureImages` and `createBranchImages` — hand an
...
// resolves through the same second path; `agent-tools.ts` owns that consumer's
// different answer to a build that fails.
//
