...
// reaches this parser. CODEX_VERSION in agent-providers.ts pins the codex-cli
// dialect whose event union is
...
// human. Captured from a live run at CODEX_VERSION with no credential:
//
...
export const codex = (model: string, options?: CodexOptions): AgentProvider => ({
  ...
  buildPrintCommand({ prompt, dangerouslySkipPermissions }) {
    ...
    // documented stdin read (verified at CODEX_VERSION: both print "Reading
  ...
  new Promise((resolveRun, rejectRun) => {
    ...
    handle
      ...
      .then((execResult) => {
        ...
        // is caught above (verified at CODEX_VERSION: no key prints `turn.failed`
