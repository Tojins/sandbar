// Shared fixtures for the gate-stack podman SHARDS, and the family's header.
//
...
//      `--health-timeout` at all (#43) — gate-stack-timeout-podman.test.ts.
//
...
//
// WHY SHARDS. This was one 2200-line file, and vitest runs a file's tests
// sequentially in one fork — so the waits these tests deliberately pay
// (mariadb bringup, readiness budgets, step timeouts) summed to ~6 minutes of
// wall clock that bounded the whole suite while every other file had long
// finished. Five files put the same tests on five workers; nothing was cut.
// #47's per-process scope is what makes that safe: each shard mints its own
// scope, so every pod, network, container and fixture tag is disjoint by
// construction and no shard can tear down another's live stack. The slices:
//
...
//                                        readiness end to end, the M5 port
//                                        probe
//   gate-stack-timeout-podman.test.ts    step timeouts and their reaps (#26),
//                                        the killed-client fact, `podman
//                                        healthcheck run` itself (#43)
...
// The worktree the stack shards gate: one commit, a marker the steps read, and
// an ignored `out/` for the tests that write artifacts without dirtying the
// tree. Caller removes it in `afterEach`.
