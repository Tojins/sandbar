  ...
  it("names a failure whose message is missing or the wrong type", () => {
    for (const line of [
      ...
      expect(parseCodexJsonLine(line)).toEqual([
        { type: "failure", kind: "provider", message: "no message" },
      ]);
    }
  });
  ...
  it.each(CODEX_REFRESH_FAILURE_LITERALS)(
    ...
    (message) => {
      expect(parseCodexJsonLine(JSON.stringify({
        type: "turn.failed",
        error: { message },
      }))).toEqual([{ type: "failure", kind: "credential", message }]);
    },
  );
...
describe("createSandbox integration (local provider)", () => {
  ...
  it("turns a permanent Codex refresh failure into a credential error ahead of quota", async () => {
    const branch = "sandbar/issue-134-codex-credential";
    await git(["branch", branch], dir);
    const detail = CODEX_REFRESH_FAILURE_MESSAGES[1];
    const line = JSON.stringify({ type: "turn.failed", error: { message: detail } });
    const sandbox = await createSandbox({
      env: {}, branch, sandbox: makeLocalProvider(), layout: layoutFor(dir),
    });
    try {
