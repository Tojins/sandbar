// The git fact behind chunk membership (#93): landing-only member refs under
// `refs/remotes/origin/sandbar/member-*` count when contained by fetched origin
// chunk refs. Commit messages are never consulted, and an ordinary parked
// `origin/sandbar/issue-*` ref is deliberately not a membership record even if
// it points at the chunk tip. These tests run git because containment and ref
// enumeration are git's contracts, not planner decisions a fake can establish.
...
describe("chunk membership from branch containment (#93)", () => {
  it("reads contained origin member refs without consulting commit messages", async () => {
    const repo = await newRepo("sandbar-chunk-members-");
  ...
  it("retains members after a chunk is fast-forwarded into source", async () => {
    const repo = await newRepo("sandbar-landed-members-");
  ...
  it("reports inherited member refs in every containing chunk", async () => {
    const repo = await newRepo("sandbar-inherited-members-");
