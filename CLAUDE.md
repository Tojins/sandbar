...
2. **Phase 2 — Inner loop** (`src/inner-loop.ts` + `src/inner-loop-machine.ts`): each planned issue runs in parallel (`Promise.allSettled`) inside its own agent sandbox + per-issue DB sidecar + podman network. The issue worktree is prepared **before** the sandbox/sidecar pair starts (the sidecar's `initMounts` bind-mount fixture files from it). The inner loop is ralph-style — up to `maxImplAttempts` (default 8) attempts in the **same** sandbox so commits accumulate on the issue branch.
...
3. **Phase 3 — Merge** (`src/merger.ts` + `src/resolve-loop.ts` + `src/merger-worktree.ts` + `src/forge-verify.ts`): procedural. Runs in a **dedicated ephemeral worktree** detached at `origin/<sourceBranch>` (`createMergerWorktree`), never the operator's primary checkout — so uncommitted operator edits can't be swept into a merge commit (issue #10). For each DONE branch in ascending issue order: capture `preMergeSha`, `git merge --no-ff`, then either succeed and push, or invoke the **agentic resolve loop** for conflict OR post-merge-gate-red. The resolve loop sees the bodies of all sibling issues in the cycle so it can reason about cross-branch intent collisions. On `<promise>ABANDON</promise>` or budget exhaustion, the merge is reverted to `preMergeSha` and the branch is skipped. The operator's local branch fast-forwards on their next pull.
   ...
   **How the cycle lands is `config.mergeMode` (#22).** `direct` (default) is a single `git push origin HEAD:<sourceBranch>` at the end. `verified` gives the forge the last word (`src/forge-verify.ts`): force-push the merge result to a scratch `integrationBranch`, poll **that sha's** check runs (sha-scoped, newest run per name — a branch-scoped or stale-run verdict must be structurally unreachable), and only green earns `git push origin <sha>:<sourceBranch>`. Red feeds the failing jobs' logs to the resolve loop in `forge-red` mode for up to `VERIFY_MAX_ROUNDS`; the loop's own local gate is then only a cheap pre-filter between expensive CI rounds, never the verdict. Checks that never conclude, or never appear at all, **park** — never land on unknown state. Verification failure is a **cycle-level** outcome (the forge judged the composed result; the red isn't attributable to one of several individually-gated merges), so the whole cycle resets to the sha the merger worktree started at, `merged` comes back empty, and every merged issue is skipped as `forge-unverified` with a comment saying it may not be the one at fault. A rejected push is `fatal` → `MergerError` → loud halt, not a parked issue. Landing is always a fast-forward of locally-authored commits, so `openPullRequest` changes attribution not at all — the PR is an audit/review handle the forge marks merged on its own.
...
- **Per-issue isolation.** Every issue gets its own podman network + DB sidecar (named `sandbar-net-<id>` / `sandbar-db-<id>`); the gate container joins the network and reaches the sidecar by its pinned IP (no host port, no name resolution — the network is `--disable-dns`, see #18). `cleanupOrphanContainers` sweeps stale prefixed resources at start and between cycles. The naming prefixes are centralized in `src/naming.ts`, which also lists the legacy `sandcastle-*`/`sandcastle/*` prefixes the sweep/clean paths still recognize during the transition window.
- **Container runtime is podman.** Hard-coded `RUNTIME = "podman"` in `db-sidecar.ts`; the agent sandbox uses the in-house `podman()` provider (`src/agent-sandbox.ts`).
- **The DB sidecar is config-driven (#20).** `config.dbSidecar` (REQUIRED — the engine is repo identity, no built-in preset) supplies image, container env, port, readiness argv, optional `containerArgs`/`initMounts`/`postReadyCommands`, and the `gateEnv` record the gate injects verbatim. Two gate-env keys are reserved and always set by sandbar: `DB_HOST` (the sidecar's pinned IP) and `DB_PORT`. `initMounts` host paths resolve against the **issue worktree** (branch-local fixtures gate against their own version), which is why the worktree is created before the sidecar starts. Images must be fully qualified (hosts without unqualified-search registries can't resolve bare short names).
...
- **Verified merge mode is about independence, not coverage (#22).** The local gate and the code under test share one process, one podman, one image cache, one env file and one author — nothing disagrees. CI is a second, differently-authored implementation of "does this work". So a coverage gap belongs in `gateCommands` (cheaper, per-attempt); `mergeMode: "verified"` exists for the thing expanding the gate can't buy. Its non-negotiable invariant: **no unknown verdict ever lands.** `integrationBranch` is a remote-only scratch ref (never a local branch — `sandbar/integration` deliberately doesn't match the `sandbar/issue-*` globs the preflight cleanup and orphan sweeper use).
...
- **Promise-token contract.** Agents signal state with a single `<promise>TOKEN</promise>` tag. Implementer: `COMPLETE` | `NEEDS-INFO` (paired with `<questions>`) | `NEEDS-UI-PROTOTYPE` (paired with `<ui-impact>`). Resolve-loop: `COMMITTED` | `ABANDON` (paired with `<reason>`). Anything else is a no-signal — the loop re-prompts. The orchestrator gates *between* attempts; the agent never decides "this is green".
